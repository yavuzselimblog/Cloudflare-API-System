...
function alert($mesaj = "The operation is successful. You are being redirected",$alert = "success"){
    echo '<div class="col-md-12 alert alert-'.$alert.'">'.$mesaj.'</div>';
}
...
function go($par,$times = false){

    if($times == false){
        $redirect = header('Location:'.$par);
    }else{
        $redirect = header('Refresh:'.$times.';url='.$par);
    }
